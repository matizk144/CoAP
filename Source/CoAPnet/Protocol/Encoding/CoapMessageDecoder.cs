...
internal sealed class CoapMessageDecoder(ILogger logger, string endpointId)
{
    private readonly CoapMessageOptionFactory _optionFactory = new();
    ...
    private CoapMessageOption CreateOption(CoapMessageOptionNumber number, byte[] value)
    {
        ...
        logger.LogWarning("{EndpointId}: {Module}: Invalid message: CoAP option {Number} not supported", endpointId, nameof(CoapMessageDecoder), number);
